...
class AuditModelAllocationViewModel(app:Application):AndroidViewModel(app){
    ...
    fun setChairman(modelId:String){_chairman.value=modelId;settings.setAuditChairman(modelId);qualify(modelId)}
    fun clearChairman(){_chairman.value="";settings.setAuditChairman("")}

    fun qualifySelectedTeam(){val ids=(_reviewers.value.filter{it.isNotBlank()}+_chairman.value).distinct();if(ids.isEmpty())return;viewModelScope.launch{_loading.value=true;try{for(id in ids){qualifyNow(id)};_message.value=if(valid())"Selected audit team passed live repository-audit qualification." else "One or more selected models are not audit-ready. Change them or use Auto-select qualified team."}finally{_loading.value=false}}}
    fun autoSelectRecommended(){viewModelScope.launch{_loading.value=true;try{val candidates=_models.value.sortedWith(compareByDescending<OpenRouterModel>{it.isFree}.thenByDescending{auditReady(it.id)}.thenBy{it.name.lowercase()});val ready=mutableListOf<String>();for(model in candidates.take(24)){val q=qualifyNow(model.id);if(q?.ready==true&&model.id !in ready)ready+=model.id;if(ready.size>=3)break};if(ready.size<2){_message.value="Fewer than two audit-qualified models are currently usable with the configured provider accounts.";return@launch};saveReviewers(ready.take(2));setChairman(ready.getOrElse(2){ready.first()});_message.value="Qualified audit team allocated. Zero-cost models are preferred when available."}finally{_loading.value=false}}}

    private fun qualify(modelId:String){viewModelScope.launch{qualifyNow(modelId)}}
    private suspend fun qualifyNow(modelId:String):AuditModelQualification?{val model=_models.value.firstOrNull{it.id==modelId}?:return null;val q=qualifier.qualify(model);_readiness.value=_readiness.value+(modelId to q);loadHealth();return q}
    ...
    fun healthLabel(id:String):String=when{ id.isBlank()->"Not assigned";_readiness.value[id]?.ready==true->_readiness.value[id]?.reason?:"Audit-ready";_readiness.value[id]?.ready==false->_readiness.value[id]?.reason?:"Audit qualification failed";auditReady(id)->"Audit-qualified";(modelHealth(id)?.consecutiveFailures?:0)>0->"Recently failing";else->"Not yet audit-qualified"}
